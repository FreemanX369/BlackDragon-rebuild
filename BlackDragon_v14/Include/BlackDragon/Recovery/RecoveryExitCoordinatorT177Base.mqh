         ...
         if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY)
         {
               ...
               Recovery_ExitExternalDealReason(reason))
            {
               ...
               if(!HistoryDealSelect(trans.deal))
               {
                  ...
                  Log_Error("Recovery","T17.26 api=HistoryDealSelect deal="+(string)trans.deal+
                            " error="+(string)apiError);
               }
               string eventContext="T17.26 phase=RUNTIME classifier=UNPROVEN_EXTERNAL deal="+(string)trans.deal+
                  " position="+(string)HistoryDealGetInteger(trans.deal,DEAL_POSITION_ID)+
                  " reason="+(string)reason+" owner="+(string)ownerMagic+
                  " programmedSL="+DoubleToString(HistoryDealGetDouble(trans.deal,DEAL_SL),_Digits)+
                  " fill="+DoubleToString(HistoryDealGetDouble(trans.deal,DEAL_PRICE),_Digits);
               Log_Error("Recovery",eventContext);
