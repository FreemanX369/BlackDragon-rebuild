...
private:
   ...
   bool ReconcileReceipt(const ulong deal,const bool deleted,const bool liveEvent,string &why)
   {
      ...
      if(deleted){if(!known){why="delete outside retained receipt proof";return false;}next=old;next.deleted=true;}
      else
      {int state=ReadReceipt(deal,liveEvent,next,why);if(state<0)return false;if(state==0){if(!known)return true;next=old;next.deleted=true;}}
      if(known&&BD_ReceiptEqual(old,next))return true;
      // Failure blocks readiness. Never persist a half-applied correction.
      if(known&&!ApplyReceiptEffect(old,-1,why))return false;
      if(!ApplyReceiptEffect(next,1,why))return false;
      if(!m_persistence.Put(next)){why="receipt capacity/validation";return false;}
      if(!deleted)TrackCursor(next.dir==0?recovery_CORE_BUY:recovery_CORE_SELL,deal);
...
public:
   ...
   void OnTick(const EAContext &ctx)
   {
      if(RecoveryMode_==recovery_ACTIVE&&m_ready&&!m_persistenceBlocked&&ctx.now>=m_overlapAt)
      {
         ...
         {m_persistenceBlocked=true;LatchReconcile(recovery_CORE_BUY,overlapWhy);LatchReconcile(recovery_CORE_SELL,overlapWhy);return;}
         if(m_dirty&&!Save(overlapWhy))return;
      }
