...
int Recovery_ArcsCopyObservation(const int coreDir,const int kind,const int generation,SArcsPosition &out[])
{
   int n=0;SBDPositionRecord r;
   if(ArrayResize(out,g_bdObservationBook.Count(),64)!=g_bdObservationBook.Count())return -1;
   for(int i=0;i<g_bdObservationBook.Count();i++)
   {
      if(!g_bdObservationBook.OrderedAt(i,r))return -1;
      if(kind==2){if(r.role!=2||1-r.dir!=coreDir||r.generation!=generation)continue;}
      else if(r.role==2||r.dir!=coreDir||(kind==0&&r.role==1))continue;
      if(r.units<=0)continue;
      SArcsPosition p;ZeroMemory(p);p.ticket=r.ticket;p.positionId=r.id;p.openTime=r.openTime;p.units=r.units;p.lots=r.lots;
      p.openPrice=r.openPrice;p.sl=r.sl;p.tp=r.tp;p.floatingCash=r.profit+r.swap;p.generation=r.generation;out[n++]=p;
   }
                           ...
                           SArcsPosition &out[])
{
   ...
   if(step <= 0.0) return 0;
   if(g_bdObservationBook.Matches(_Symbol,(long)Magic,(long)RecoveryMagic_,step))
      return Recovery_ArcsCopyObservation(dir==recovery_CORE_BUY?0:1,0,0,out);
   long wanted = Recovery_ArcsCoreType(dir);
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong ticket = PositionGetTicket(i);
                                   ...
                                   SArcsPosition &out[])
{
   ...
   if(step <= 0.0) return 0;
   if(g_bdObservationBook.Matches(_Symbol,(long)Magic,(long)RecoveryMagic_,step))
      return Recovery_ArcsCopyObservation(dir==recovery_CORE_BUY?0:1,1,0,out);
   long wanted = Recovery_ArcsCoreType(dir);
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong ticket = PositionGetTicket(i);
                                     ...
                                     SArcsPosition &out[])
{
   ...
   if(g_bdObservationBook.Matches(_Symbol,(long)Magic,(long)RecoveryMagic_,step)&&!g_bdObservationBook.HasUnknownGeneration())
      return Recovery_ArcsCopyObservation(dir==recovery_CORE_BUY?0:1,2,generation,out);
                             ...
                             const int generation,const double step)
{
   ...
   if(g_bdObservationBook.Matches(_Symbol,(long)Magic,(long)RecoveryMagic_,step)&&!g_bdObservationBook.HasUnknownGeneration())
      return g_bdObservationBook.LayerUnits(dir==recovery_CORE_BUY?0:1,generation);
                                  ...
                                  datetime &openTime)
{
   ...
   if(g_bdObservationBook.Matches(_Symbol,(long)Magic,(long)RecoveryMagic_,step))
   {
      int at=0;SBDPositionRecord r;
      for(int i=0;i<g_bdObservationBook.Count();i++)
      {
         if(!g_bdObservationBook.OrderedAt(i,r))return false;
         if(r.role!=0||r.dir!=(dir==recovery_CORE_BUY?0:1)||r.units<=0)continue;
         if(at++!=RecoveryStartAfterDca_)continue;
         positionTicket=r.ticket;price=r.openPrice;openTime=r.openTime;return true;
      }
      return false;
   }
   SArcsPosition core[];
