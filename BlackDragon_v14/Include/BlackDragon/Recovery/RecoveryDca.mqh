...
// One fatal stop request per EA instance; preserve the first cause.
bool g_recoveryTesterStopRequested=false;
void Recovery_StopTesterOnStartupBlock(const string scope,const bool runtime=false)
{
   if(!MQLInfoInteger(MQL_TESTER) || g_recoveryTesterStopRequested) return;
   g_recoveryTesterStopRequested=true;
   string phase=runtime?"RUNTIME":"INIT";
   Log_Error("Recovery", "Strategy Tester stopped: phase="+phase+" cause=RECOVERY_NOT_READY; Recovery ACTIVE is not reconciled/ready (" + scope + ")");
   TesterStop();
}
...
private:
   ...
   bool m_seenReady;
public:
   CRecoveryStartupFilter(CRecoveryEngine *recovery) { m_recovery = recovery;m_seenReady=false; }
   bool Allow(const EAContext &ctx, const int dir)
   {
      ...
      if(m_recovery == NULL)
      {
         ...
         Recovery_StopTesterOnStartupBlock("new-series engine unavailable",m_seenReady);
         return false;
      }
      if(!m_recovery.ActiveReady())
      {
         ...
         Recovery_StopTesterOnStartupBlock("new-series gate",m_seenReady);
         return false;
      }
      m_seenReady=true;
      return true;
...
public:
   ...
   bool Allow(const EAContext &ctx, const int dir)
   {
      ...
      if(m_recovery == NULL || m_basket == NULL)
      {
         ...
         Recovery_StopTesterOnStartupBlock("DCA adapter unavailable",m_seenReady);
         return false;
      }
      if(!m_recovery.ActiveReady())
      {
         ...
         Recovery_StopTesterOnStartupBlock("DCA gate",m_seenReady);
         return false;
      }
