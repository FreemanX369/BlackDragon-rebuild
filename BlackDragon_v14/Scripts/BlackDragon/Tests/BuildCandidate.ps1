...
function Compile-ExactSource([string]$SourcePath,[string]$Name) {
    ...
    if ($text -notmatch '\b0 errors?,\s*0 warnings?\b' -or $text -match ' : (error|warning) ' -or -not (Test-Path $ex5)) { throw "Compile gate failed: $Name; inspect $log" }
...
if ($tests.Count -ne 34) { throw "Expected 34 native test sources; found $($tests.Count). Update the enrolled contract before building." }
