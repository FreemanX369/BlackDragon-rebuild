...
# T17.25 receipt fixture in t1725_cases.py supersedes the cursor adapter.
# Keep the old adapter for an explicitly requested historical counterexample.
replay_rc=0
...
position_header=re.sub(r'\b(\w+) &(\w+)\[\]',r'std::vector<\1> &\2',position_header)
position_header=re.sub(r'\b(\w+) (\w+)\[\];',r'std::vector<\1> \2;',position_header)
position_header=position_header.replace('int m_ids[],m_order[],m_work[];','std::vector<int> m_ids,m_order,m_work;')
...
enum {POSITION_TIME=299,POSITION_SYMBOL=300,POSITION_MAGIC,POSITION_TYPE,POSITION_VOLUME,POSITION_PRICE_OPEN,POSITION_COMMENT,POSITION_IDENTIFIER,POSITION_SL,POSITION_TP,POSITION_SWAP,POSITION_PROFIT};
...
int OC_RhGeneration(const string&){return 1;}
template<class T>void ZeroMemory(T &x){x=T{};}
enum{BD_M_SORT_ITEMS};struct Metrics{void Add(int,ulong=1){}}g_bdMetrics;
...
int main(){
 ...
 SBDPositionRecord record;SBDPositionAggregate aggregate;
 T1724Check("T1725 immutable-ID index",book.ById(1002,record)&&record.ticket==2&&record.role==1);
 T1724Check("T1725 scope generation aggregate",book.Aggregate(2,1,1,aggregate)&&aggregate.units==40&&aggregate.count==1);
 T1724Check("T1725 weighted numerator",std::abs(aggregate.weightedNumerator-1200)<1e-9);
 T1724Check("T1725 unknown ID cannot alias",!book.ById(9999,record));
 T1724Check("T1725 immutable ordering tie",book.OrderedAt(0,record)&&record.ticket==1);

 book.End();
 T1724Check("Q55 snapshot unavailable at mutation boundary",!book.Matches("fixture",100,200,.01));
 T1724Check("T1725 no indexed data outside observation",!book.ById(1002,record)&&!book.OrderedAt(0,record));
