...
ck('ExpectedRhTrim(deal)' in arcs and 'layer.realizedOtherCash+=sign*r.cash' in arcs and
   'layer.realizedFundingCash+=sign*r.cash' in arcs,'RH trim cash is separate from RH funding cash')
ck('layer.tpBaselineUnits-=sign*r.units' in arcs,
...
ck('DEAL_TIME_MSC' in arcs and 'stamps[i]==stamps[j]&&replay[i]<replay[j]' in arcs,
   'replay is deterministically sorted by time_msc and ticket')
ck('ReconcileReceipt(replay[i],false,false,why)' in arcs and 'BD_ReceiptEqual(old,next)' in arcs and 'TrackCursor(next.dir' in arcs,
   'replay exact-once receipt replaces cursor-only dedup')
