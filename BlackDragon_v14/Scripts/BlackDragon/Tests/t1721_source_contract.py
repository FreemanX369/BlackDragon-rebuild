...
from t1725_diagnostic_baseline import before_diagnostics
...
for path,sha in MANIFEST['original_sha256'].items():
    ...
    text=before_comments(path,before_diagnostics(path,(REPO/path).read_text()))
...
for path,sha in contract['protected_source_sha256'].items():
    if path in superseded_t1722: continue
    ck(hashlib.sha256(before_diagnostics(path,(REPO/path).read_text()).encode()).hexdigest()==sha,'unchanged: '+Path(path).name)
