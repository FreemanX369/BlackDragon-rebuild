...
ck('stamps[i]==stamps[j]&&replay[i]<replay[j]' in replay,'stable timestamp/ticket replay ordering')
ck('history correction requires funding-ledger reconciliation' in arcs,'correction does not leave funding silently valid')
...
ck('m_retry[d].reconcile!=(m_retry[d].rejects>=8)' in protect,'loaded exhausted budget requires reconciliation')
save=body(protect,'bool Save()','bool HeaderMatches(')
ck('if(!LoadedStateValid(h))' in save,'writer validates payload before replacing disk state')
ck('if(!FileSeek(f,(long)sizeof(SPyProtectDisk),SEEK_SET))' in protect,'payload read checks seek result')
adx=rd(INC/'Filters/AdxFilter.mqh')
ck('!MathIsValidNumber(adx[0]) || adx[0]==EMPTY_VALUE' in adx,'ADX rejects nonfinite and empty data before threshold')
...
ck('OrderSend' not in book and 'FileWrite' not in book and 'bool ById(' in book and 'm_indexValid' in book,'indexed observation has no broker mutation and rebuilds invalid indexes')
ck('m_enabled=false' in book and 'PositionsTotal()!=total' in book,'partial observation falls back to live reads')
